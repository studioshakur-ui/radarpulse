-- Remove EU->IT normalization from opportunities_search_v1 and enforce true country codes.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'opportunities_country_code_it_eu_check'
      and conrelid = 'public.opportunities'::regclass
  ) then
    alter table public.opportunities
      add constraint opportunities_country_code_it_eu_check
      check (country_code in ('IT', 'EU'));
  end if;
end $$;
