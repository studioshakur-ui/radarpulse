...
const log = createLogger("worker/uk_sell2wales");

type Sell2WalesResponse = Record<string, unknown> | unknown[] | null;
...
function firstRegionHint(release: any): string | null {
  ...
  for (const item of asArray(release?.tender?.items)) {
    const region = firstTruthyString([
      item?.deliveryAddresses?.[0]?.region,
  ...
  for (const party of asArray(release?.parties)) {
    const region = firstTruthyString([party?.address?.region]);
    if (region) return region;
  }
...
function extractReleases(data: Sell2WalesResponse): any[] {
...
export async function apiSell2WalesFetch(source: SourceRow): Promise<ConnectorResult> {
  const fetched_at = new Date().toISOString();
  const meta = (source.meta ?? {}) as Record<string, unknown>;
  const locale = Number(meta.locale ?? 2057) || 2057;
  ...
  for (const dateFrom of monthCursors) {
    for (const noticeType of noticeTypes) {
      ...
      const { data } = await safeJsonFetch<Sell2WalesResponse>(url.toString(), {
        method: "GET",
        headers: {
