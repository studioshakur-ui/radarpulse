...
const log = createLogger("dossiers");

type DossierStatus = "NEW" | "REVIEW" | "GO" | "HOLD" | "BLOCKED" | "READY" | "SUBMITTED";
type DossierAction =
  | "create_dossier"
  | "get_my_dossiers"
  | "get_dossier"
...
function normalizeStatus(value: unknown): DossierStatus | null {
  return value === "NEW" ||
      value === "REVIEW" ||
      value === "GO" ||
      value === "HOLD" ||
...
function flattenOpportunity(value: DossierRow["opportunities"]): OpportunityJoinRow | null {
  if (!value) return null;
  if (Array.isArray(value)) return value[0] ?? null;
  return value;
}

function serializeDossier(row: DossierRow) {
  return {
    id: row.id,
...
async function maybeSyncStatus(
  sb: ReturnType<typeof sbAdmin>,
  dossier: DossierRow,
  desiredStatus: DossierStatus | null,
): Promise<DossierRow> {
  if (!desiredStatus) return dossier;
  if (dossier.status === desiredStatus) return dossier;

  const canAutoAdvance =
    (desiredStatus === "GO" || desiredStatus === "HOLD") &&
    (dossier.status === "NEW" || dossier.status === "REVIEW" || dossier.status === "HOLD" || dossier.status === "GO");

  if (!canAutoAdvance) return dossier;
...
Deno.serve(async (req) => {
  ...
  try {
    body = (await req.json()) as DossierRequest;
  } catch {
  ...
  try {
    if (action === "create_dossier") {
      ...
      let dossierRow = existing as DossierRow | null;
      if (!dossierRow) {
        ...
        if (insertError) {
          ...
          const raced = await loadDossierRow(
            sb,
          ...
          dossierRow = inserted as DossierRow;
        }
      }
      if (!dossierRow) throw new Error("Failed to resolve dossier.");
      dossierRow = await maybeSyncStatus(sb, dossierRow, desiredStatus);

      await seedTasksFromPrepIfEmpty(sb, dossierRow.id, dossierRow.opportunity_id, userId);
    ...
    if (action === "get_my_dossiers") {
      ...
      return json(200, {
        ok: true,
        items: ((data as DossierRow[] | null) ?? []).map(serializeDossier),
      });
    }

    if (action === "get_dossier") {
      const dossierId = normalizeString(body.dossier_id);
      if (!dossierId) return json(400, { ok: false, error: "INVALID_DOSSIER_ID" });

      const dossier = await loadDossierRow(sb, dossierId, userId);
      if (!dossier) return json(404, { ok: false, error: "DOSSIER_NOT_FOUND" });
      ...
      const [{ data: tasks, error: tasksError }, { data: decision, error: decisionError }] = await Promise.all([
        ...
        sb
          ...
          .eq("opportunity_id", dossier.opportunity_id)
          .eq("user_id", userId)
          .maybeSingle(),
      ]);
      ...
      return json(200, {
        ok: true,
        dossier: serializeDossier(dossier),
        tasks: (tasks as DossierTaskRow[] | null) ?? [],
        decision: decision?.decision ?? null,
        decided_at: decision?.decided_at ?? null,
      });
    }

    if (action === "update_dossier_status") {
      const dossierId = normalizeString(body.dossier_id);
      const status = normalizeStatus(body.status);
      if (!dossierId || !status) return json(400, { ok: false, error: "INVALID_BODY" });
      ...
      if (error) throw new Error(error.message);
      if (!data) return json(404, { ok: false, error: "DOSSIER_NOT_FOUND" });

      return json(200, {
        ok: true,
        dossier: serializeDossier(data as DossierRow),
      });
    }
    ...
    if (action === "add_task") {
      ...
      if (!dossierId || !label) return json(400, { ok: false, error: "INVALID_BODY" });
    ...
    if (action === "toggle_task") {
      ...
      const dossier = await loadDossierRow(sb, String(task.dossier_id), userId);
      if (!dossier) return json(404, { ok: false, error: "DOSSIER_NOT_FOUND" });
    ...
    log.error("dossiers_handler_error", {
      user_id: userId,
      action,
